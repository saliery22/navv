...
unitslist.forEach(function(unit) {          
    ...
    if((Date.now())/1000-parseInt(sdsa.t)>3600 || parseInt(sdsa.sc)<5){
                         if((Date.now())/1000-parseInt(sdsa.t)>21600){
                          let markerstarton = L.marker([sdsa.y, sdsa.x],{interactive: false, icon: L.icon({iconUrl: "stop.png",iconSize:[12,12],iconAnchor:[6, 6]})}).addTo(map);
                          online_mark[unit.getId()] = markerstarton;
                         }else{
                           if(parseInt(sdsa.sc)<5){
                           let markerstarton = L.marker([sdsa.y, sdsa.x],{interactive: false, icon: L.icon({iconUrl: "stop3.png",iconSize:[12,12],iconAnchor:[6, 6]})}).addTo(map);
                           online_mark[unit.getId()] = markerstarton;
                           }else{
                             if((Date.now())/1000-parseInt(sdsa.t)>3600){
                             let markerstarton = L.marker([sdsa.y, sdsa.x],{interactive: false, icon: L.icon({iconUrl: "stop2.png",iconSize:[12,12],iconAnchor:[6, 6]})}).addTo(map);
                             online_mark[unit.getId()] = markerstarton;
                             }
                         }
                         }
           ...
           if(online_mark[unit.getId()]) map.removeLayer(online_mark[unit.getId()]);
            if((Date.now())/1000-parseInt(pos.t)>3600 || parseInt(pos.sc)<5){
                         if((Date.now())/1000-parseInt(pos.t)>21600){
                          let markerstarton = L.marker([pos.y, pos.x],{interactive: false, icon: L.icon({iconUrl: "stop.png",iconSize:[12,12],iconAnchor:[6, 6]})}).addTo(map);
                          online_mark[unit.getId()] = markerstarton;
                         }else{
                           if(parseInt(pos.sc)<5){
                           let markerstarton = L.marker([pos.y, pos.x],{interactive: false, icon: L.icon({iconUrl: "stop3.png",iconSize:[12,12],iconAnchor:[6, 6]})}).addTo(map);
                           online_mark[unit.getId()] = markerstarton;
                           }else{
                             if((Date.now())/1000-parseInt(pos.t)>3600){
                             let markerstarton = L.marker([pos.y, pos.x],{interactive: false, icon: L.icon({iconUrl: "stop2.png",iconSize:[12,12],iconAnchor:[6, 6]})}).addTo(map);
                             online_mark[unit.getId()] = markerstarton;
                             }
                         }
                         }
            ...
            let markerstarton = L.marker([pos.y, pos.x],{interactive: false, icon: L.icon({iconUrl: "stop_onl.png",iconSize:[10,10],iconAnchor:[-8, 5]})}).addTo(map);
            markerstarton.setRotationAngle(parseInt(pos.c)-90);
            online_mark[unit.getId()] = markerstarton;
            }
            }
...
let zone20km_yx =[[52.358828590091186,32.33551025390626],[52.39403946901232,33.84338378906251],[51.81201099369774,34.45587158203126],[51.32374658474385,34.45861816406251],[51.32460477256678,33.970756530761726],[51.40520274223228,33.92578125000001],[51.42147659056747,33.89831542968751],[51.46812470399791,33.87943267822266],[51.494637234974235,33.88629913330079],[51.493140970308815,33.91397953033448],[51.51245503991427,33.91814231872559],[51.5148854147594,33.88904571533204],[51.547829565965294,33.894882202148445],[51.55444755572847,33.85608673095704],[51.593282143771575,33.85711669921876],[51.59797418707659,33.83274078369141],[51.63421454302622,33.809738159179695],[51.650193736726834,33.94672393798829],[51.671490569495546,33.94569396972657],[51.67351324801491,33.97994041442872],[51.6866052450109,33.97968292236329],[51.74403752566788,33.87084960937501],[51.7710267510184,33.850936889648445],[51.851897666096086,33.92166137695313],[51.91970865511044,33.83377075195313],[52.04995693234416,33.73901367187501],[52.07190953840937,33.67584228515626],[52.12843000062979,33.65386962890626],[52.119998657638156,33.452682495117195],[52.15961210128133,33.27827453613282],[52.10270941733637,33.09356689453126],[52.10102230322338,32.991256713867195],[52.071065406906634,32.93975830078126],[52.06937709602395,32.62939453125001]];
             ...
             jurnal(1,Vibranaya_zona);
               ...
               if ($('#geomodul').is(':hidden')==false){
                if($('#name_pole').text()!=Vibranaya_zona.n){
                  ...
                  let polilane = L.polyline(ramka, {color: 'blue'}).addTo(map);
                  geo_layer.push(polilane);
                  $('#obrabotkaBT1').css({'background':'#00ff80' });
                  $('#obrabotkaBT2').css({'background':'#f0f0f0' });
                  $('#obrabotkaBT3').css({'background':'#f0f0f0' });
                  Naryady_start();
...
treeselect5 = new Treeselect({
 ...
 treeselect_geomodul = new Treeselect({
  parentHtmlContainer: document.querySelector('.container_geomodul'),
  value: ['John Deere'],
  options: [{ "value": -1, "name": 'ВСІ АВТО', children: serch_list_avto }],
  staticList: true,
  saveScrollPosition:true,
  ...
  showTags: true,
  tagsCountText: "вибрано",
  placeholder: "ОБЕРІТЬ ТЕХНІКУ ДЛЯ ОБРОБІТКУ",
});
...
//=================Geomodul===================================================================================
$('#obrabotkaBT1').click(function() {
  this.style.background = '#00ff80';
  $('#obrabotkaBT2').css({'background':'#f0f0f0' });
  $('#obrabotkaBT3').css({'background':'#f0f0f0' });
  Naryady_start(1);
});
$('#obrabotkaBT2').click(function() {
  this.style.background = '#00ff80';
  $('#obrabotkaBT1').css({'background':'#f0f0f0' });
  $('#obrabotkaBT3').css({'background':'#f0f0f0' });
  Naryady_start(2);
});
$('#obrabotkaBT3').click(function() {
  this.style.background = '#00ff80';
  $('#obrabotkaBT2').css({'background':'#f0f0f0' });
  $('#obrabotkaBT1').css({'background':'#f0f0f0' });
  Naryady_start(3);
...
function Naryady_start(ver){

   str=''; 
   treeselect_geomodul.value.forEach((element) => {str+=element.split('|||')[1]+',';});
   str = str.slice(0, -1);
   if(!str)return;
  Naryady(Global_DATA,str,ver);
  clear();
 ...
 let geo_layer =[];
 let geo_splines = [];
function Naryady(data=[],maska='JD',ver){
...
function myroutine(){
      ...
      if(i == geo_splines.length-1){
          ...
          for(let i = table_polya.rows.length-ns; i<table_polya.rows.length; i++){
             for(let ii = i+1; ii<table_polya.rows.length; ii++){

                if(table_polya.rows[i].cells[2].textContent==table_polya.rows[ii].cells[2].textContent && table_polya.rows[i].cells[3].textContent==table_polya.rows[ii].cells[3].textContent && table_polya.rows[i].cells[4].textContent==table_polya.rows[ii].cells[4].textContent && table_polya.rows[i].cells[5].textContent==table_polya.rows[ii].cells[5].textContent && table_polya.rows[i].cells[6].textContent==table_polya.rows[ii].cells[6].textContent && table_polya.rows[i].cells[7].textContent==table_polya.rows[ii].cells[7].textContent && table_polya.rows[i].cells[8].textContent==table_polya.rows[ii].cells[8].textContent){
                  let obr = parseFloat(table_polya.rows[i].cells[10].textContent)+parseFloat(table_polya.rows[ii].cells[10].textContent);
                  let fakt = parseFloat(table_polya.rows[i].cells[11].textContent)+parseFloat(table_polya.rows[ii].cells[11].textContent);

                  let json1 = JSON.parse(table_polya.rows[i].cells[14].textContent);
                  let json2 = JSON.parse(table_polya.rows[ii].cells[14].textContent);

                  json2.coordinates.forEach(function(spline) { json1.coordinates.push(spline) });
                 
                  table_polya.rows[i].cells[10].textContent=obr.toFixed(1);
                  table_polya.rows[i].cells[11].textContent=fakt.toFixed(1);
                  table_polya.rows[i].cells[14].textContent=JSON.stringify(json1);
                  table_polya.deleteRow(ii);
                  ii--;
                }
             }
          }

          table_polya.scrollIntoView({ block: 'end' });
      }
  }
...
for(let i = 0; i<geozonesgrup.length; i++){ 
 ...
 function zlivy(){
  ...
  for(let i = 0; i<Global_DATA.length; i++){
    let nametr = Global_DATA[i][0][1];
    let id = Global_DATA[i][0][0];
    if(nametr.indexOf('ДРП Криски')>=0 || nametr.indexOf('ДРП ККЗ')>=0 || nametr.indexOf('ДРП Райгородок')>=0 || nametr.indexOf('Бензин ККЗ Ультразвук')>=0 || nametr.indexOf('РЕЗЕРВУАР ККЗ новий')>=0 || nametr.indexOf('знято')>=0 ||  nametr.indexOf('Знято')>=0 ||  nametr.indexOf('еревантажувач')>=0) continue;
    let start=0;
    let finish=0;
    let interval0=0;
    let interval1=0;
...
$('#polya_kkz').click(function() {
  ...
  for (let i = 0; i<geozones.length; i++){
    ...
    $("#unit_table").append("<tr><td>"+(i+1)+"</td><td>"+geozones[i].zone.n+"</td><td>"+(geozones[i].zone.ar/10000).toFixed(1).toString().replace(/\./g, ",")+"</td><td>"+geozones[i].gr+"</td><td>"+ct+"</td><td>"+mt+"</td><td>"+geozones[i].zone.d+"</td><td style='background-color:"+geozones[i].options.fillColor+";'>"+geozones[i].options.fillColor+"</td></tr>");
  }
  });
    ...
    for (let ii = 1; ii < data[i].length; ii++) {
      ...
      if (line.length > 0) {
        ...
        if (dis > 20000 || dis <= 0) {
          ...
          if( Date.parse(data[i][ii][1])>=fr)  km += dis;
        }
      }
